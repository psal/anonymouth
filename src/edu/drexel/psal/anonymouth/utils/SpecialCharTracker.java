...
public class SpecialCharTracker implements Serializable {
	...
	public int[] extendHighlights(int start, int end, int[] allSentIndices, int[] sentenceLengths, int selectedSentence) {
		...
		for (int i = 0; i < NUM_OF_TEXT_WRAPPERS; i++) {
			highlightIndices = extendHighlights( allSentIndices, sentenceLengths, selectedSentence, allTextWrappers.get(i), allTextWrapperSizes[i], highlightIndices);
		}
		
		return highlightIndices;
	}

	private int[] extendHighlights(int[] allSentIndices, int[] sentenceLengths,
