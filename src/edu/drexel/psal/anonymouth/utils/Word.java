...
public class Word implements Comparable<Word>, Serializable {
	...
	public double getWordAnonymity() {
		...
		for (i = 0;i<numFeatures;i++){
			...
			// for 'getPercentChangeNeeded', the first boolean says not to normalize the result to the baslinePercentChangeNeeded, the second says to invert the percentage, and the third says to take the absolute value (ignore direction of change)
			anonymityIndex += (value)*(currentAttrib.getInfoGain())*(currentAttrib.getPercentChangeNeeded(false,false,false));
		}
	...
	public String toString(){
		return "[ WORD: "+word+" ||| Anonymity Index: "+getWordAnonymity()+" ||| wordLevelFeaturesFound: "+wordLevelFeaturesFound+"]";
	}
	...
	public int compareTo(Word notThisWord) {
		double thisAnonIndex = this.getWordAnonymity();
		double thatAnonIndex = notThisWord.getWordAnonymity();
