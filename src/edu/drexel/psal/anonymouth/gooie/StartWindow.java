...
public class StartWindow extends JFrame {
	...
	public void loadProblemSet(String path) {
		...
		try {
			...
			main.preProcessWindow.ps.setDummyAuthor("~* you *~");
			...
			if (main.preProcessDriver.updateAllComponents() && probSetReady) {
				...
				if (!ANONConstants.IS_MAC) { // We do this so it works for both OS X and Windows
					...
					trainDocFilePath = trainDocFilePath.replace('/', '\\');
				}
				main.preProcessDriver.updateOpeningDir(testDocFilePath, false);
				main.preProcessDriver.updateOpeningDir(trainDocFilePath, true);
			} else {
