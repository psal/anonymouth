...
public class AnonymityBar extends JPanel {
	...
	private void updateTubeFill() {
		...
		topY = barTubeY;
		curPercent = 1 - newValue; //In Anonymouth, this is where you get an calculate the percent (range 0-100%, you have to adjust it)
		curHeight = (int)((bottomY-topY)*curPercent + 0.5);

		updateColor();
		
		//Obtaining the string we will use for displaying the percent text
		BigDecimal bd = new BigDecimal(Double.toString(curPercent*100));
	...
	public void updateBar() {
		...
		String doc; 
		do {
			doc = main.documentPane.getText();
		} while(doc.isEmpty());
		String pathToTempModdedDoc = ANONConstants.DOC_MAGICIAN_WRITE_DIR + "AB" + ".txt";
		List<Document> toModifySet = new LinkedList<Document>();
			...
			Document newModdedDoc = new Document(pathToTempModdedDoc,"","AB");
		...
		Logger.logln(NAME+"CurPercent = " + curPercent*100 + ": " + (1 - newValue) + " / 1");
	}

	public double getAnonymityBarValue() {
		return newValue;
	}
