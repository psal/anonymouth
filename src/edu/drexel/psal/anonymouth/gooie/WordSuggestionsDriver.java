...
public class WordSuggestionsDriver {
	...
	public void placeSuggestions() {
		...
		//get full list of word suggestions
		HashMap<String, ArrayList<String[]>> result =  ConsolidationStation.getWordToRemoveAndWordToAdd();

		try {
			//-----------------HANDLING WORDS TO REMOVE-------------------
			topToRemove = result.get("wordsToRemove");
			...
			for (int i = 0; i < removeSize; i++) {
				if (!topToRemove.get(i).equals("''") && !topToRemove.get(i).equals("``")) {
					...
					if (topToRemove.get(i)[1].equals("1"))
						right = "1 time";
					else
			...
			topToAdd = result.get("wordsToAdd");
			...
			for (int i = 0; i < addSize; i++) {
				boolean add = false;
				String curWord = topToAdd.get(i)[0];
				...
				if (add) {
					...
					if (curWord.equals(prevSelectedElement)) {
						main.elementsToAddPane.setSelectedValue(curWord, true);
					}
				} else {
					bad.add(curWord);
				}
