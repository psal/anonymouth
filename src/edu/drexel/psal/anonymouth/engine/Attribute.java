...
public class Attribute {
	...
	public void setGenericName(){
		...
		*/
		...
		genericName = FeatureList.valueOf(tempFeatName.replaceAll("[ -]","_").toUpperCase());
	...
	public double getPercentChangeNeeded(boolean normalizeToBaseline, boolean invertPercentage, boolean takeAbsVal) {
		double temp = 0;
		double perm = 0;
		// the smallest change that you can make to a feature and still have it be a whole feature (an integer). 
		//For example, if you have 5 of the letter (feature) ‘a’, then (100/5)/100 = .2  so, the smallest increase of the letter ‘a’ is 20%. 
		double minimumPercentChangeUnit = 0;
		double halfOfMin;
		//the remainder you get from dividing the actual percent change needed by the smallest allowed percent change. If temp (the percent change we need) is less than the percent change we want, 
		//then theModulus will be equal to temp (look up modulus (“mod” and “%” mean the same thing) if you don’t understand why. As some examples: 10 % 50 = 10,  50 % 10 = 0, and  7 % 2 = 1
		double theModulus;
