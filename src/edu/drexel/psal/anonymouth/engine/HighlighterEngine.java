...
public class HighlighterEngine {
	...
	/**
	 * Clears all words to add and words to remove highlights.
	 */
	...
	public void addAllRemoveHighlights(String wordToHighlight) {
		...
		for (int i = 0; i < indexSize; i++) {
			...
			try {
				if (index.get(i)[0] == 0) { // check if start index is at the very beginning
					if (text.charAt(index.get(i)[1]) != '-')
						realWord = true;
				} else if (index.get(i)[1] == text.length() - 1) { // check if end index is at the very end
					if (text.charAt(index.get(i)[0] - 1) != '-')
						realWord = true;
				} else // check normal cases
					if (text.charAt(index.get(i)[0] - 1) != '-' && text.charAt(index.get(i)[1]) != '-')
						realWord = true;
				if (realWord)
